...
async function populateEverywhere() {
    console.log('--- RE-POPULATING ALL RAIL BENCKMARKS FROM LATEST PROMPT ---');
    const rails = [
    ...
    const categories = ['guardRail_type', 'wallRail_type', 'grabRail_type', 'caneRail_type'];

    try {
        for (const [label, lbs, shop, field] of rails) {
            for (const cat of categories) {
                const [rows] = await query('SELECT id FROM dictionary WHERE label = ? AND category = ?', [label, cat]);
                
                if (rows && rows.length > 0) {
                    console.log(`[UPDATE] ${cat} | ${label} -> ${lbs}, ${shop}, ${field}`);
                    await query('UPDATE dictionary SET steelLbsLf = ?, shopLaborMhLf = ?, fieldLaborMhLf = ? WHERE id = ?', [lbs, shop, field, rows[0].id]);
                } else {
                    console.log(`[INSERT] ${cat} | ${label} -> ${lbs}, ${shop}, ${field}`);
                    await query('INSERT INTO dictionary (label, value, category, steelLbsLf, shopLaborMhLf, fieldLaborMhLf) VALUES (?, ?, ?, ?, ?, ?)', 
                        [label, label, cat, lbs, shop, field]);
                }
            }
        }
