...
async function run() {
  try {
    console.log('Seeding all rail dictionary categories...');
    ...
    for (const [category, items] of Object.entries(SEED_DATA)) {
      ...
      for (const item of items) {
        ...
        await db.query(`INSERT INTO dictionary (category, label, value, [order], steelLbsLf, shopLaborMhLf, fieldLaborMhLf, isActive) 
                        VALUES ('${category}', '${item.label.replace(/'/g, "''")}', '${autoValue}', ${addedCount + 1}, ${item.lbs}, ${item.shop}, ${item.field || 0}, 1)`);
        addedCount++;
      }
      console.log(`Seeded ${addedCount} entries for ${category}.`);
    }
    
    console.log('Final cleanup of duplicates...');
    // Optional: remove any other categories that might be confused? No, stick to these.
