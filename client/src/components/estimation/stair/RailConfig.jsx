...
export default function RailConfig({ type = 'guardRail', data, onChange }) {
  ...
  return (
    <div>
      ...
      {/* ── Real-time Preview Engine Results ─────────────────────── */}
      {data?.systemCalc && (
        <div style={{ marginTop: 24, display: 'grid', gap: '16px' }}>
          <div className="summary-card card-glow-purple" style={{
            ...
            {data.systemCalc?.steelPerLF ? (
              <div style={{ display: 'grid', gap: '20px' }}>
                {/* 🛡️ SFE PER UNIT SECTION (EXCEL ALIGNED) */}
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, paddingBottom: 16, borderBottom: '1px dashed #CBD5E1' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '18px', fontWeight: 800, color: '#0F172A' }}>{Number(data.systemCalc.steelPerLF || 0).toFixed(3)}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    <div style={{ fontSize: '9px', fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 4, lineHeight: '1.2' }}>{config.hasPosts ? 'Posts' : (config.hasBrackets ? 'Brackets' : 'Spans')}</div>
                    <div style={{ fontSize: '15px', fontWeight: 800, color: '#0F172A' }}>{config.hasPosts ? (data.systemCalc.posts || 0) : (config.hasBrackets ? (data.systemCalc.bracketQty || 0) : 0)}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '9px', fontWeight: 700, color: '#10B981', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 4, lineHeight: '1.2' }}>Mounting<br />Charge</div>
                    <div style={{ fontSize: '15px', fontWeight: 800, color: '#059669' }}>${Number(data.systemCalc.mountingCharge || 0).toFixed(2)}</div>
                  </div>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 16 }}>
                  ...
                  <div>
                    <div style={{ fontSize: '9px', fontWeight: 700, color: '#F59E0B', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 4 }}>Scrap Weight (11%)</div>
                    <div style={{ fontSize: '18px', fontWeight: 800, color: '#D97706' }}>{Number(data.systemCalc.scrapLbs || 0).toFixed(3)}</div>
                  </div>
                  <div title="Formula: Benchmark × 1.11 Fittings/Scrap Factor">
                    <div style={{ fontSize: '9px', fontWeight: 700, color: '#8B5CF6', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 4 }}>STEEL (+11% SCRAP) LBS</div>
                    <div style={{ fontSize: '18px', fontWeight: 800, color: '#7C3AED' }}>{Number(data.systemCalc.finalScrapWeight || data.totalWeight || 0).toFixed(3)}</div>
                  </div>
                  <div>
                  ...
                  <div>
                    <div style={{ fontSize: '9px', fontWeight: 700, color: '#8B5CF6', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 4 }}>Galvanize Cost</div>
                    <div style={{ fontSize: '18px', fontWeight: 800, color: '#7C3AED' }}>${Number(data.systemCalc.galvanizeTotalCost || 0).toFixed(2)}</div>
                  </div>
                </div>
              </div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12 }}>
                <div>
                  <div style={{ fontSize: '9px', fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 }}>
                    {config.hasPosts ? 'Posts' : (config.hasBrackets ? 'Brackets' : 'Anchor Points')}
                  </div>
                  <div style={{ fontSize: '18px', fontWeight: 800, color: '#0F172A' }}>
                    {config.hasPosts ? (data.systemCalc?.posts || 0) : (config.hasBrackets ? (data.systemCalc?.bracketQty || 0) : 0)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '18px', fontWeight: 800, color: '#0F172A' }}>
                    {Number(data.systemCalc.actualSpacing || 0).toFixed(3)}
                    <span style={{ fontSize: '10px', opacity: 0.7, marginLeft: 2 }}>ft</span>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div style={{ fontSize: '18px', fontWeight: 800, color: data.toeplateRequired === 'Yes' ? '#10B981' : '#CBD5E1' }}>
                    {data.toeplateRequired === 'Yes' ? `${data.systemCalc.toePlateWeight || 0} lb` : 'N/A'}
                  </div>
                </div>

                <div title="Formula: (Rails + Posts + Fittings + Toe) × 1.08 × 1.11">
                  <div style={{ fontSize: '9px', fontWeight: 700, color: '#8B5CF6', textTransform: 'uppercase', letterSpacing: '0.5px', marginBottom: 4 }}>
                    Est. Fabricated Weight
                  </div>
                  <div style={{ fontSize: '18px', fontWeight: 800, color: '#0F172A' }}>
                    {Number(data?.totalWeight || 0).toFixed(3)}
                    <span style={{ fontSize: '10px', opacity: 0.7, marginLeft: 2 }}>lb</span>
                  </div>
                </div>
              </div>
            )}
