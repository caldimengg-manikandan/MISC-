...
export default function SFEEstimateReport({ data, onBack }) {
  ...
  // Rates from config
  const pricePerLb = config.steel_price_per_lb;
  const shopLaborRate = config.shop_hourly_rate;
  const fieldLaborRate = config.field_hourly_rate;
  ...
  const scrapWeight = summary.scrapWeight || (steelWeight * 0.11); 
  ...
  const steelPriceRaw = summary.baseSteelCost || (steelWeight * pricePerLb);
  const scrapPriceRaw = summary.scrapWeightCost || (scrapWeight * pricePerLb);
  const shopLaborPrice = summary.shopLaborCost || (shopHours * shopLaborRate);
  const fieldLaborPrice = summary.fieldLaborCost || (fieldHours * fieldLaborRate);
  ...
  // Matches exact calculation format generated by the master dashboard
  const totalMaterialPrice = steelPriceRaw + pansPrice + gratingPrice + galvanizePrice + mountingCharges;
  
  // PARITY: Link specifically to backend structured objects so grand logic matches exactly with DB
      ...
      <div ref={printRef} className="report-paper bg-white mx-auto shadow-2xl overflow-hidden relative" style={{ width: '1056px', minHeight: '816px', padding: '20px' }}>
        ...
        <div className="flex justify-between items-start mb-6">
          <div className="flex items-center gap-4">
            {config.company_logo ? (
                 <img src={config.company_logo} alt="Company Logo" style={{ maxWidth: '180px', maxHeight: '100px', objectFit: 'contain' }} />
            ) : (
